...
class FlightSearch:
    ...
    def check_flights(self, origin_city_code, destination_city_code, from_time, to_time, is_direct):
        ...
        Parameters:
            ...
            is_direct (bool): Whether or not the departure is direct or not.
        ...
        query = {
            ...
            "adults": 1,
            "nonStop": f"{is_direct}".lower(),
            "currencyCode": "GBP",
            "max": "10",
        }
        print("flight parameters", query)
        response = requests.get(
            url=FLIGHT_ENDPOINT,
            headers=headers,
            params=query,
        ...
        data = response.json()
        if (data is None or not data['data']) and is_direct:
