...
def admin_only(f):
    ...
    def wrapper(*args, **kwargs):
        if is_admin():
            print("Running admin only function")
...
def show_post(post_id):
    ...
    comments_request = db.session.execute(db.select(Comment).where(Comment.post_id == requested_post.id))
    comments = comments_request.scalars().all()
    ...
    return render_template("post.html", post=requested_post, comment_form=comment_form, comments=comments, gravatar_url=gravatar_url)
